...
export function registerRepoTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ projectId, location, repositoryGroup, indexId, kmsKey }) => {
      try {
        ...
        return {
          content: [{ type: 'text', text: stdout || `Index ${indexId} creation initiated.` }],
        };
      } catch (error: any) {
  ...
  server.tool(
    ...
    async ({ projectId, location, indexId, groupId, repositories }) => {
      try {
        const targetProject = await getProjectId(projectId);
        const repoConfig = repositories
          .map((r) => `resource=${r.resource},branchPattern=${r.branchPattern}`)
          .join(',');
        const stdout = await runGcloud([
          'gemini',
          'code-repository-indexes',
        ...
        return {
          content: [{ type: 'text', text: stdout || `Group ${groupId} creation initiated.` }],
        };
      } catch (error: any) {
  ...
  server.tool(
    'grant_repository_group_access',
    {
      ...
      role: z
        .string()
        .default('roles/cloudaicompanion.repositoryGroupsUser')
    ...
    async ({ projectId, location, indexId, groupId, email, role }) => {
      try {
        ...
        return {
          content: [{ type: 'text', text: `Granted ${role} to ${email} on group ${groupId}.` }],
        };
      } catch (error: any) {
