            ...
            try {
                ...
                if (!targetBillingAccount || !targetOrder) {
                    ...
                    try {
                        ...
                        if (projectBilling.billingAccountName) {
                            ...
                            if (!billingAccounts.some((a: any) => a.name.endsWith(projectAccountId))) {
                                billingAccounts.push({ name: projectBilling.billingAccountName, open: projectBilling.billingEnabled });
                            }
                        }
                    } catch (e) {}
                    ...
                    for (const account of billingAccounts) {
                        ...
                        try {
                            const ordersStdout = await runCurl(['-H', `Authorization: Bearer ${token}`, '-H', `X-Goog-User-Project: ${targetProject}`, ordersUrl]);
                    ...
                    if (!targetBillingAccount || !targetOrder) {
                        const explanation = await sampleMessage(server, "Explain how to find Billing Account and Order ID for Gemini Code Assist in Cloud Console.");
                        return {
                            content: [{ type: 'text', text: explanation || 'Could not find GCA order automatically.' }]
                        };
                    }
                }
                ...
                const stdout = await runCurl(['-H', `Authorization: Bearer ${token}`, '-H', `X-Goog-User-Project: ${targetProject}`, url]);
        ...
        async ({ billingAccountId, orderId, projectId, email }) => {
            try {
                ...
                const stdout = await runCurl([
                    ...
                    '-H', `X-Goog-User-Project: ${targetProject}`,
